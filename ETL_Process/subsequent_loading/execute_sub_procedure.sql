...
--
--   EXEC load_date_dim_incremental(2023);                <- the year
--   EXEC maintain_product_dim_scd2(DATE '2023-01-01');   <- price-rise date
--   EXEC load_order_fact_incremental(DATE '2022-01-01'); <- backfill window
--
-- ===================================================================
...
-- reach 2023 BEFORE they run, or every 2022-23 transaction fails its
...
-- Skip the date step and 189,747 order lines vanish with nothing to
-- show for it.
--
-- ===================================================================
-- PREREQUISITES
...
-- CHANGE THE YEAR HERE if you extend past 2023. This must run before
-- the facts, or their date lookups fail silently.
EXEC load_date_dim_incremental(2023);

-- The new 2022-2023 days arrive with holiday_ind = 'N'. AFTER this
...
-- prices in data22_23\99_price_increase_2023.sql went up, so the expired
-- version ends 2022-12-31 and the new one starts 2023-01-01. The 2022
-- lines in data22_23 still carry the OLD price and resolve to the
-- expired version by order date; the 2023 lines carry the new one. Date it
-- today instead and five years of order lines would be attributed to
...
-- procedure filters src_date >= p_load_date - 1, so DATE '2022-01-01'
-- opens the window at 2021-12-31. That one extra day is already in the
-- fact and the NOT EXISTS anti-join skips it.
--
-- The window has no upper bound, so one call backfills all of data22_23.
--
...
-- expect 2192 for 2018-2023 (2,191 days + the Unknown member)
...
GROUP BY yr
ORDER BY yr;
-- 2018-19 baseline, 2020-2021 dip (lockdowns), 2022 recovers, 2023 grows on.
