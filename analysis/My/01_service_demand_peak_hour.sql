...
--
-- SCOPE NOTE
--   Only res_status = 'Completed' rows count here (consistent with
--   Fz\01_branch_profitability.sql's measure definitions). Cancelled /
--   No-Show behaviour is a deliberately separate question - see a
--   Reservation Reliability report for that angle.
--
-- MEASURES  (reservation_fact, Completed lines only)
--   Reservations  = COUNT(res_det_ID)
--   Revenue (RM)  = serv_price - serv_discount_amt, EXCLUDING tax (SST
...
--                drill path (year -> month -> day of week)
--   branch_dim   br_city / br_name                 -> section 3, filter
--   staff_dim    st_role                            -> section 5
...
--   plus reservation_fact.start_hour, the column purpose-built for
--   peak-hour analysis.
--
-- REPORT SECTIONS (the drill-down path)
--   1  RESERVATION DEMAND PER YEAR          2019-2025 overview
...
WITH yearly AS (
    SELECT d.cal_year,
           ...
           SUM(f.serv_price - f.serv_discount_amt)   AS revenue,
...
BREAK ON REPORT
COMPUTE SUM LABEL 'ALL MONTHS' OF y2019 y2020 y2021 y2022 y2023 y2024 y2025 total_res ON REPORT
...
WITH dow AS (
    SELECT d.day_week, d.day_num_week, d.weekday_ind,
           COUNT(f.res_det_ID)                    AS num_reservations,
           SUM(f.serv_price - f.serv_discount_amt) AS revenue
    FROM   reservation_fact f
    JOIN   date_dim   d ON d.date_key   = f.date_key
    ...
    AND   (UPPER(TRIM('&branch')) IN ('', 'ALL')
           OR UPPER(b.br_name) LIKE '%' || UPPER(TRIM('&branch')) || '%')
    GROUP  BY d.day_week, d.day_num_week, d.weekday_ind
)
...
WITH banded AS (
    SELECT CASE WHEN f.start_hour BETWEEN 9  AND 11 THEN 1
                WHEN f.start_hour BETWEEN 12 AND 16 THEN 2
                WHEN f.start_hour BETWEEN 17 AND 20 THEN 3
                ELSE 4 END AS band_order,
           CASE WHEN f.start_hour BETWEEN 9  AND 11 THEN 'Morning (9am-12pm)'
                WHEN f.start_hour BETWEEN 12 AND 16 THEN 'Afternoon (12pm-5pm)'
                WHEN f.start_hour BETWEEN 17 AND 20 THEN 'Evening (5pm-9pm)'
                ELSE 'Other Hours' END AS hour_band,
           f.staff_key, s.st_role, f.res_det_ID,
...
SELECT band_order, hour_band,
       ...
       COUNT(DISTINCT CASE WHEN st_role IN ('Beauty Therapist', 'Senior Therapist')
...
WITH lines AS (
    SELECT d.cal_year, d.cal_month_name, MOD(d.cal_year_month, 100) AS month_num,
           d.day_week, d.day_num_week,
           f.start_hour, f.staff_key, f.res_det_ID,
           f.serv_price - f.serv_discount_amt          AS revenue
    FROM   reservation_fact f
    JOIN   date_dim   d ON d.date_key   = f.date_key
    JOIN   branch_dim b ON b.branch_key = f.branch_key
...
by_dow_focus AS (
    SELECT day_week, day_num_week, COUNT(res_det_ID) AS num_res
    FROM   lines WHERE cal_year = &focus_year GROUP BY day_week, day_num_week
),
by_hour_focus AS (
